<?php
    ...
    if ($_SESSION['fight']['ennemi']->speed > $_SESSION['perso']['vitesse']) {
        ...
        if ($touche >= 10) {
            $_SESSION['fight']['html'][] = "Vous touchez votre ennmi.";
            $degat = random_int(0,10) + floor($_SESSION['perso']['force']/3);
            ...
            if ($_SESSION['fight']['ennemi']->pol <= 0) {
                ...
                if ($touche >= 10) {
                    $_SESSION['fight']['html'][] = "Il vous touche.";
                    $degat = random_int(0,$_SESSION['fight']['ennemi']->power) + floor($_SESSION['fight']['ennemi']->power/3);
                    $_SESSION['fight']['html'][] = "Il vous inflige " . ($degat - floor($_SESSION['perso']['force']/3)) . " dégats";
                    $_SESSION['perso']['point'] -=  $degat - floor($_SESSION['perso']['force']/3);
                } else {
    ...
    $bdd = connect();
    $sql = "UPDATE persos SET `gold` = :gold, `point` = :point WHERE id = :id AND user_id = :user_id;";    
    $sth = $bdd->prepare($sql);

    $sth->execute([
        'gold'      => $_SESSION['perso']['gold'],
        'point'       => $_SESSION['perso']['point'],
    ...
    if ($_SESSION['perso']['point'] <= 0) {
        $_SESSION['fight']['html'][] = "Vous etes mort.";
        $_SESSION['fight']['html'][] = "<button onclick='location.href=\"persos.php\"'>Choisir un autre personnage</button>";
        $_SESSION['fight']['html'][] = "<button onclick='location.href=\"index.php\"'>Quitter le combat</button>";
    } else {
        // le code existant pour le tour de combat
    }
